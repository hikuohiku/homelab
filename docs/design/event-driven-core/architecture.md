...
- **D13a. 認証は NKey（2026-08-23 実装）**: 当初の実装はパスワード方式にしてしまい、
  NATS 自身が `Plaintext passwords detected` と警告した（自分で決めた D13 の基準を
  実装で下げていた）。誰も接続していないうちに NKey へ移した。`nats.conf` に載るのは
  **公開鍵**（秘密ではないので Git に置ける）で、seed は Doppler からクライアントへ配る。
  秘密そのものはネットワークに出ない（nonce への署名だけが飛ぶ）。
