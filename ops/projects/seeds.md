...
16. **curriculum PR にも見張り時限を** — 2026-08-09、立案 PR #418 が CI の一過性エラー
    (チャート配布元の 500) で red のまま 15 時間、無通知で放置された (プロジェクト PR の
    merge_timeout はあるが curriculum PR には無い)。時限 + 通知に加え、「変更が ops/ のみの
    PR で落ちた kustomize build」のような無関係な一過性失敗は CI 再実行を一度試みる
17. **自己コメントの再摂取を除外する** — outbox の digest を issue #56 に代送すると、
    次のビートの collect_feedback がそれを人間の声として review_needed に数える
    (「(通知予算超過分のまとめ」プレフィクスの除外が無い)。無害だが帳簿が濁る
18. ~~【最優先・人間の鍵作業】器のアイデンティティ分離~~ — **人間が意図的に却下 (2026-08-09)**。
    理由:「PR 通知が増えて面倒。git で復元でき、バックアップの壁は健在」。壁 #7 (ハーネス保護) は
    存在しないことを**仕様として受容**する。器の実効的な歯止めは独立レビュー + CI + soak + veto +
    append-only バックアップ。**このシードを再提案しないこと** (状況が変わった場合のみ、変化を明記して新シードとして)
    — (旧記載) — 2026-08-09、ハーネス保護
