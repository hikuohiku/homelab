...
- **追記（同一起動内、webhook で #153 merge を検知後）**: #153 は構築セッションのレビュー
  （issue #56 05:12:43、設計は健全と評価。RBAC が get/list のみで secret 権限なし、非 root、
  専用ブランチへの書き戻し、pip install 不要を評価）を経て hikuohiku アカウントにより merge された。
  cooldown の趣旨（レビュー窓の確保）は同一起動内でも別主体のレビューが挟まれば満たされると判断し、
  T-0015 を `done` にした。同コメントで指摘された不足（PVC の実使用量・コンテナの実メモリ/CPU 使用量が
  取れない）を T-0076（du ベースの補完 CronJob、追加 RBAC 不要）・T-0077（metrics-server 経由、
  `metrics.k8s.io` の get/list のみ）として起票した
- **もう1件、issue #56 05:16:46 の指摘**: cooldown（PR を開いたまま起動を終える）と直列化
  （前のタスクの PR が merge されるまで次のブランチを作らない）が両立しないという構造的な指摘。
  cooldown 中の PR に `ops/` の記録（backlog のタスクステータス等）まで載せると、その PR が
  未マージの間は次のタスクが記録を引き継げない。CHARTER §4 に対処（cooldown PR には記録を載せず
  別 PR に分ける）を追記した。今回は #152（他セッション）が自分で base を更新して衝突を解消済みだった
  ため実害は無かったが、次回以降はこの運用に従うこと
- 前回時点からの差分: PR #152 の mergeable_state は `blocked`（CI 待ち、コンフリクトではない）に
  変わっていた。他セッションが自分で `main` を取り込んで解消していたので、こちらからの介入は不要だった
