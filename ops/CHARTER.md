...
| 6 | CI green を確認して auto-merge に載せる |
...
- **1 PR = 1 論点。** 「ついでに直した」を混ぜない。見つけた別の問題は backlog に起票する
  - 例外は `ops/` の運用記録（journal / backlog / state / dashboard）。これは論点ではなく作業の副産物なので、
    その回の PR に相乗りさせる。`main` へ直 push できない以上、記録もここに載せるしかない
...
その起動は何も残さずに丸ごと無駄になる。実際に run #1 がこれで消えた。

以下は**絶対に実行しない**（`.claude/settings.json` と `harness.toml` の `ask` / `deny` 対象）。

| コマンド | 代わりにどうするか |
|---------|------------------|
| `kubectl`（全サブコマンド） | クラスタには到達できないので元々不要。**マニフェストの検証は CI に任せる**。手元で確かめたいときは `kustomize build --enable-helm <dir>`（無ければ検証は CI 任せでよい） |
| `rm -r` | `git clean` や個別の `rm` を使う。そもそも消す必要があるか考える |
| `git push --force` | 使わない。やり直したいならブランチを作り直す |
| `sudo` | 使わない（deny） |

`just` のレシピの多くは内部で `kubectl` を呼び、クラスタへの到達を前提にしている。
**`just preflight` / `just preview*` / `just plan` / `just apply` は実行しない。**
