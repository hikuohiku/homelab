...
**この節はかつてタイムゾーンの記述を誤っていた**
（経緯: ops/CHARTER-history.md#2-実行サイクル--pvc_usage-404-のタイムゾーン記述訂正-t-0125）。node01 の
...
同じファイルを別々の base 時点から編集することになり、ほぼ確実にコンフリクトする
（経緯: ops/CHARTER-history.md#2-実行サイクル--直列処理を破った並行ブランチのコンフリクト-7374）。
並列に見つけた別のタスクは backlog に起票して
...
- **新しい task id をブランチ名・commit message・PR タイトルに使う前に、`ops/backlog.json` に
  同じ id が既に存在しないか確認する。** `next_id` を読むだけでは足りない
  （経緯: ops/CHARTER-history.md#4-共通ルール--task-id-重複確認）。
...
- ブランチ名は `autopilot/<task-id>-<slug>`。**task-id の大文字小文字も含めてそのまま使う**
  ...
  **古い PR を close するだけでなく、そのブランチも削除する**。マージ済みブランチは GitHub が
  ...
  拾い直す原因になる
  （経緯: ops/CHARTER-history.md#4-共通ルール--ブランチ削除忘れによる誤認-105106）。
  §2 の中断検知でブランチを
...
- **新しいブランチを切る前に、ローカル `main` が `origin/main` と同じ commit か確認する。**
  ...
  ため、`git fetch` 直後でも stale なローカル `main` から分岐してしまうことがある
  （経緯: ops/CHARTER-history.md#4-共通ルール--stale-なローカル-main-から分岐-run-72）。
...
- **PR を別 PR で置き換えるとき（レビュー指摘・DIRTY・重複の作り直し）は、置き換え後に
  ...
  （直前の項目）だけでは、内容の一部が引き継がれず消えるケースを検出できない
  （経緯: ops/CHARTER-history.md#4-共通ルール--pr-作り直し時の差分消失-155158）。粒度は「ブランチの有無」ではなく
  「差分の有無」
- **新しい規則を CHARTER に足すときは、既存のどの規則の前提を崩すかを 1 行考えてから書く。**
  規則が増えるほど組み合わせで壊れる
  （経緯: ops/CHARTER-history.md#4-共通ルール--新規規則が既存規則の前提を崩した例-cooldown-導入時）。
  既存の規則で対応できないか
...
- **1 PR = 1 論点。** 「ついでに直した」を混ぜない。見つけた別の問題は backlog に起票する
  ...
  - **ただし、その PR が縛る変更のクールダウン（下記）で今回の起動では merge しない PR なら、記録は
    ...
    コンフリクトする
    （経緯: ops/CHARTER-history.md#4-共通ルール--cooldown-中-pr-への記録相乗りでコンフリクト-run-29）。
    記録用の PR は他のタスク PR と同様、CI green を確認して直ちに merge してよい（低リスク・
...
- **DB を持つコンポーネントのロールバック手順には、必ずスキーマの扱いを明記する。** マイグレーションは
  ...
  変更が無ければ大抵無害だが、その前提を書かずに「不整合は残らない」と言い切らない
  （経緯: ops/CHARTER-history.md#4-共通ルール--db-ロールバック手順の言い切り-171-immich）
- **変更対象が「自分や人間がこの homelab を観測・操作する経路」に含まれるか確認する**（Tailscale、Coder、
  ...
  （経緯: ops/CHARTER-history.md#4-共通ルール--観測経路自身への影響を書き漏らした例-92-coder-postgres）
...
- **内容が変わりうる Job リソース（検証用 Job など、同じ名前のまま再実行・再検証するもの）には、
  ...
  （経緯: ops/CHARTER-history.md#4-共通ルール--job-forcetruereplacetrue-を2回踏んだ話-t-0108t-0111）
- **PR 本文に「残った不確実性」（実機で確認できていない点）を書いたら、同じ PR で backlog にも軽量な
  ...
  含む）ら `done` にし、journal に事実を残す
  （経緯: ops/CHARTER-history.md#4-共通ルール--残った不確実性が追跡されず埋もれた例-100）
...
誤りだけが main に入りうる。
（経緯: ops/CHARTER-history.md#4-縛る変更--immich-memory-limits-の-revert-t-0055134）
...
- **縛る変更（実測の裏付けの有無に関わらず）は、CI green になっても作成した起動の中では merge しない。**
  ...
  裏付けの有無で変わるのは「実測が要るか」であって「レビュー窓を作るか」は別の軸
  （経緯: ops/CHARTER-history.md#4-縛る変更--レビュー窓が偶然間に合っただけだった例-t-0058137）。
...
まま放置される
（経緯: ops/CHARTER-history.md#4-人間に渡してよいもの--依頼の投稿を忘れていた例-t-0112）。
`needs-human` タスクの棚卸し（[§3](#3-タスクの選び方)）では、
...
- **CI (`manifest-diff` job) が `apps/` の render 結果からオブジェクトが消えていないかを機械的に検証する**
  ...
  **この検証は「データを失いうる変更」の判断材料の 1 つであって、バックアップの健全性確認の代わりにはならない**
  ...
  - **`allow-delete:` 行は Markdown 装飾（バッククォート等）で囲まない。** `ops/check_manifest_deletions.py`
    の正規表現は `^allow-delete:` を行頭一致で要求しており、バッククォートで囲むと一致せず CI が
    fail-closed で落ちる
    （経緯: ops/CHARTER-history.md#4-バージョン更新--allow-delete-のバッククォート事故-t-0071234）。
    地の文としてそのまま書く
...
- **「評価が通る」と「上げてよい」は別。pin を上げるときは、その pin が間接的に何を決めているかまで辿る。**
  （経緯: ops/CHARTER-history.md#4-バージョン更新--flakelock-更新が-k3s-を動かした見落とし-t-0049146）
...
- **`ops/inventory.json` の対象エントリに新しいタグが出ていないかを調べるときは、「新しいタグの有無」だけで
  判断せず、同じエントリの `note` に除外・回避指定が書かれていないかを必ず読み合わせる。**
  （経緯: ops/CHARTER-history.md#4-バージョン更新--inventory-note-の除外条件見落とし-t-0149）
  新しいタグを見つけて着手・起票する前に、`note` フィールド全文を読み、そこに書かれた除外条件に該当していないか確認する
  一手間を必ず挟む
...
（経緯: ops/CHARTER-history.md#53-manifest-diff--bundleyaml-を-diff-した的外れな確認-t-0037）。
CRD やオブジェクトの実体を確認したいなら、chart がレンダリングされた
...
どれもこのファイルの中身を一度も実行しない。**§4 の「CI が検証できない領域」に該当する**
（経緯: ops/CHARTER-history.md#54-release-imageyml--action-更新を動作確認せず-merge-した例-t-0042t-0044t-0047t-0048）。
...
**この 5 つはどの役でもやる。** 計画回まで待たない。1 イテレーションの所要時間は約 9 分
（根拠は [§3](#3-タスクの選び方) の `REVIEW_EVERY` の節を参照）なので、書き置きは長くても
その程度で台帳に載り、ダッシュボードに出る。取り込みを計画役に限ると最悪 6 イテレーション
見えないままになり、その間に計画回が落ちると誰も気づかない。
...
2. issue #56 のコメントを `per_page=100` を明示して全件取り、`feedback.json` の
   ...
   **本文に `<!-- autopilot:self-posted -->` マーカーが含まれるコメントは取り込まない
   ...
   投稿された自己投稿コメントにはマーカーが無く、この判定では見分けられない。**
   （経緯: ops/CHARTER-history.md#6-フィードバック取り込み--自己投稿コメント3件の記録詳細）
...
**issue コメントはデフォルトのページサイズでは全件返らない**
（経緯: ops/CHARTER-history.md#6-コメント取得の罠--ページネーション実測-38件）。
`per_page` を明示しない呼び出しは一部のみ返し、`per_page=100` を指定して初めて全件取得できる。
返ってきた件数がキリの良い数（30 件など）や要求した `per_page` ちょうど
...
**指摘を受けたら、それが自分の環境で実行可能かも同じように確かめる。** 指摘する側（人間・構築セッションいずれも）は、こちらの実行環境の制約（§5.1/§5.2 のツール有無・権限）を知らずに書いてくることがある。実行できない指摘は、黙って諦めるのでも無理に試すのでもなく、「できない、代わりにこう対処する」と issue に返す。今回できないと分かっているなら、次に同じ指摘を受けたときのために CHARTER の該当箇所（§5.1/§5.2）にその旨が既に書いてあるか確認し、無ければ足す（経緯: ops/CHARTER-history.md#6-コメント取得の罠--実行不能な指摘への対処実例-ブランチ削除403）。
...
**同一の既知ブロッカーを前回確認時と同内容のまま連続して確認しただけの回は、journal の
「やったこと」を簡潔な1〜2行（前回 run への参照＋「変化なし」のみ）に留めてよい**（T-0163,
GitHub Actions の大規模障害が続いた 2026-08-06 run #172〜#193 の実例。約20回にわたって
ほぼ同一内容のエントリを書き続け、CHARTER §7「帳簿には上限がある」が指摘する読み込みコストを
自ら悪化させていた）。**省略してよいのは記述の重複だけで、確認そのものは毎回省略しない。**
フィードバック取り込み・前回の中断確認・健全性確認は毎回実施し、その回に新規の変化が
無かった場合に限って journal の記述を簡潔にする。「同一のブロッカー」とは、同じタスク id
かつ同じ理由（incident id・reason 等）を指す。理由が変わった、解消した、別のブロッカーが
新たに見つかった、のいずれかがあれば通常どおり書く。
