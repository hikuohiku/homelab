...
**「実行中かどうかを外から知れない」問題への判断**（issue #56, 2026-08-05 16:33:23 構築セッションの
提案。手動起動が cron の実行と 3 回重なり重複 PR を作った経緯を踏まえ、heartbeat 用のブランチ等を
検討すべきかという相談）。現状は重複が起きても互いに検知して close できており（run #13/#44/#46/#55
で実例）、実害は起動 1 回分の無駄で収まっている。対策（heartbeat ブランチの追加）を先に導入すると
器がまた 1 つ増え、VISION の「器を太らせる前に、器を使い切る」に反する。**当面は対策しない。**
重複の頻度や実害が増えたら改めて検討する（run #56 の判断）。
