...
{{PROJECT_BRANCH}} にいる。あなたの仕事は **PROJECT.md を作って commit する。それだけ。**
実装を始めてはいけない。
...
- PROJECT.md と PROJECT-PROGRESS.md (空でよい、見出しだけ) を作り、
  `git add PROJECT.md PROJECT-PROGRESS.md && git commit` する。push は wrapper がやる
- 後続の worker セッションは毎回フレッシュに起動され、**PROJECT.md と
  PROJECT-PROGRESS.md と git log だけを文脈として引き継ぐ**。
